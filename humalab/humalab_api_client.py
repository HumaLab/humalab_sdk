...
class HumaLabApiClient:
    ...
    def upload_code(
        ...
        """
        Upload a code artifact (text/YAML content).
        
        Args:
            artifact_key: Artifact key identifier
            run_id: Run ID
            code_content: Text content to upload (YAML, config, etc.)
        ...
        response = self.post("/artifacts/code", data=form_data)
        return response.json()
    ...
    def upload_python(
        ...
        """
        Upload a Python artifact (pickled blob data).
        ...
        with open(file_path, 'rb') as f:
            ...
            response = self.post("/artifacts/python", files=files, data=form_data)
            return response.json()
