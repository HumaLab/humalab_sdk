...
metric = hl.metrics.Metrics(
    metric_dim_type=MetricDimType.ONE_D,
...
Control how metrics are visualized:

```python
from humalab.constants import MetricDimType

# One-dimensional metrics (single value)
MetricDimType.ONE_D

# Two-dimensional metrics (x, y pairs)
MetricDimType.TWO_D

# Three-dimensional metrics (x, y, z tuples)
MetricDimType.THREE_D
```
...
Summary metrics aggregate data into a single value:
...
# Add to run
run.add_metric("total_episodes", summary)

# Log a single summary value
run.log({"total_episodes": 100})
```
...
Create custom metrics by subclassing the `Metrics` base class:
...
class CustomMetric(Metrics):
    ...
    def _finalize(self) -> dict:
        ...
        processed_values = [v * 2 for v in self._values]
